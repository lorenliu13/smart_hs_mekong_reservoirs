...
Usage
-----
  Submitted via sbatch — see submit_era5land_extraction.sbatch.
...
# ---------------------------------------------------------------------------
DAILY_DIR = Path(r"/data/ouce-grit/cenv1160/smart_hs/processed_data/mekong_river_basin_reservoirs/era5_land_daily")
CATCHMENT_SHP = (
...
OUTPUT_DIR = Path(
    ...
    rf"/era5_land_daily_catchment_level/daily_per_lake_area_{AREA_THRESHOLD_SQKM}_sample_{OBS_COUNT_THRESHOLD}sqkm"
)
...
def _build_grid_polygons(lats, lons, res=GRID_RES):
    half = res / 2.0
    rows = []
    for lat in lats:
        for lon in lons:
...
def process_variable_month(
    daily_dir, year, month, col_name, weights_dict, output_dir, overwrite=False
):

...
Inputs
------
  Daily ECMWF NetCDF files: <DATA_DIR>/<YYYY-MM>/hres_mekong_<YYYY-MM>_<col_name>_daily.nc
...
Usage
-----
  python extract_ecmwf_per_catchment.py
...
# ---------------------------------------------------------------------------
DATA_DIR = Path(r"E:\Project_2025_2026\Smart_hs\raw_data\ecmwf_ifs\hres_daily")
CATCHMENT_SHP = (
...
def daily_nc_file_path(data_dir: Path, year: int, month: int, col_name: str) -> Path:
    """Return path to a monthly ECMWF daily-aggregated NetCDF file."""
...
def load_daily_ecmwf(nc_path: Path) -> xr.Dataset | None:
    """
    Load a daily ECMWF NetCDF file produced by aggregate_ecmwf_to_daily.py.

    Returns an xr.Dataset with dims (init_time, forecast_day, latitude, longitude),
...
# ===========================================================================
# V.  Main processing loop  (one variable at a time)
# ===========================================================================
...
def main() -> None:
    ...
    # ---- 3. Determine ECMWF grid from the first available daily NetCDF ----

#!/bin/bash
#SBATCH --job-name=run_lake_spatialcv
#SBATCH --output=run_lake_spatialcv.out
#SBATCH --error=run_lake_spatialcv.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
CONFIG="configs/exp06_mekong_wse1d_era5_ifshres_gritv06_202312_202602_spatialcv.yaml"
...
for FOLD_IDX in $(seq 0 $((N_FOLDS - 1))); do
    ...
    # ── Training ───────────────────────────────────────────────────────────────
    python run_training_lake_wse1d_spatial_cv.py \
        --config          "$CONFIG" \

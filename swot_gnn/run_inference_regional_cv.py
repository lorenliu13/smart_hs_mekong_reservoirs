"""
Inference script for regional cross-validation lake WSE forecasting.

Mirrors run_inference_spatial_cv.py but for checkpoints produced by
run_training_lake_wse1d_regional_cv.py.  Key differences:

  - Datasets are rebuilt with build_regional_cv_fold (same fold / seed args
    as training) so the spatial masks are available.
  - Lakes are split into 5 geographic regions (REGIONAL_FOLD_MAP); one region
    is held out as the test set per fold.
  - Inference is run on all three splits (train, val, test).  Every row is
    ...
    spatial_mask.
  - Per-lake metrics (physical units) are computed for all splits.  The full
    result CSVs are saved for all lakes / all splits.
...
Usage:
  python run_inference_regional_cv.py \\
    ...
    --run-name        exp07_mekong_wse1d_era5_ifshres_gritv06_202312_202602_regionalcv \\
    --fold-idx        0 \\
    --val-method      spatial \\
    --spatial-val-frac 0.2 \\
    --seed            42 \\
    --device          cuda
...
name is resolved correctly (identical slug logic to run_training_lake_wse1d_regional_cv.py).
"""
import argparse
import json
import sys
...
def main():
    parser = argparse.ArgumentParser(
        description="Regional-CV inference — load checkpoint, run predictions, save CSVs"
    )
    ...
    # Mirror run-name construction from run_training_lake_wse1d_regional_cv.py
    ...
    _print_summary(f"Train lakes", lake_metrics_train_df)
    _print_summary(f"Val lakes",   lake_metrics_val_df)
    _print_summary(f"Test lakes [{region_name}]", lake_metrics_test_df)

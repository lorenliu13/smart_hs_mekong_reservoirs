#!/bin/bash
#SBATCH --job-name=run_lake_temporalcv_wsend
#SBATCH --output=run_lake_temporalcv_wsend.out
#SBATCH --error=run_lake_temporalcv_wsend.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
for FOLD_IDX in $(seq 0 $((N_FOLDS - 1))); do
    ...
    # ── Training ───────────────────────────────────────────────────────────────
    python wsend_training/run_training_lake_wsend_temporal_cv.py \
        --config          "$CONFIG" \
    ...
    python wsend_training/run_inference_wsend_temporal_cv.py \
        --config          "$CONFIG" \
        --wse-datacube    "$WSE_DATACUBE" \
        --era5-datacube   "$ERA5_DATACUBE" \
        --ecmwf-datacube  "$ECMWF_DATACUBE" \

...
Usage:
  python run_training_lake_wsend_temporal_cv.py \\
...
Launch all folds (bash example):
  for i in 0 1 2; do
    python run_training_lake_wsend_temporal_cv.py ... --fold-idx $i &
  done
"""
import argparse
import csv
...
def train(cfg, args):
    ...
    registry_path = Path(args.save_dir) / "registry_wsend_temporal_cv.csv"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Temporal expanding-window CV — multi-day lake WSE forecasting (SWOT-GNN)"
    )
    parser.add_argument(
    ...
    assert forecast_horizon > 1, (
        f"run_training_lake_wsend_temporal_cv.py is designed for forecast_horizon > 1, "
    ...
    fold_def = TEMPORAL_FOLD_DATES[args.fold_idx]
    # Encode fold, horizon, and seed in the run folder name.
    # Example: exp08_wsend_temporalcv_fold0of3_h10_swotgnn_nd_s42

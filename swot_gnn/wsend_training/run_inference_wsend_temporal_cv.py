...
Mirrors run_inference_wsend_regional_cv.py but for checkpoints produced by
run_training_lake_wsend_temporal_cv.py.  Key differences from the regional version:

  - Split is temporal, not spatial — all lakes are active in every fold.
  - No is_test_lake flag; instead each test row carries a fold_idx column so
    results from all three folds can be pooled unambiguously.
  - train / val / test rows are separated by init_date (not by lake).
  - Pooled evaluation: load test_result_df.csv from all three folds, concatenate,
    and compute one global metric over all (fold, sample, lake, lead_day) tuples.
...
Usage:
  python run_inference_wsend_temporal_cv.py \\
...
def run_inference(ds, model, device, fold_idx: int, split_name: str = "") -> pd.DataFrame:
    """
    Run model over all samples in ds; return DataFrame of observed-lake predictions.
...
def main():
    parser = argparse.ArgumentParser(
        description="Temporal-CV multi-step inference — load checkpoint, run predictions, save CSVs"
    )
    ...
    assert forecast_horizon > 1, (
        f"run_inference_wsend_temporal_cv.py is designed for forecast_horizon > 1, "
    ...
    # Mirror run-name construction from run_training_lake_wsend_temporal_cv.py
    ...
    # Each dataset already contains only its own init_date window (train / val / test),
    # so we run inference separately on each and tag every row with fold_idx.
    print("\nRunning inference ...")
    ...
    # ── Save result CSVs ──────────────────────────────────────────────────────
    # fold_idx column in test_result_df.csv marks which temporal fold each row
    # belongs to, enabling pooled evaluation across all three folds.
    col_order = [
        "lake_id", "fold_idx", "init_date", "forecast_date", "lead_day",

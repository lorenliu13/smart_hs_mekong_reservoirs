"""
Inference script for single-lead-day temporal CV lake WSE forecasting
using the LSTM-only baseline.

Mirrors run_inference_lstm_wsend_temporal_cv.py but for checkpoints produced by
run_training_lake_lstm_wsend_single_lead_cv.py.  Dataset is rebuilt with
build_temporal_cv_fold_single_lead so the split exactly matches training.

Key differences from the standard temporal CV inference script:
  - Dataset is SingleLeadDatasetLake, indexed by valid_samples (N, 2) not valid_starts.
  - Each sample has label_mask = 1 at only one lead day, so each prediction row
    corresponds to a single (init_date, lead_day) pair.
  - --lead-assign must match the value used during training.

Usage:
  python single_lead/run_inference_lstm_wsend_single_lead_cv.py \\
    ...
    --run-name        exp02_lstm_wsend_singleld \\
    --fold-idx        0 \\
    --lead-assign     round_robin \\
    --seed            42
"""
import argparse
import json
...
def run_inference(ds, model, device, fold_idx: int, split_name: str = "") -> pd.DataFrame:
    """
    Run LSTM model over all samples in ds; return DataFrame of observed-lake predictions.

    Each row records:
        lake_id, fold_idx, init_date, forecast_date, lead_day,
        pred_norm, target_norm, pred_std_norm

    Because ds is a SingleLeadDatasetLake, each sample has label_mask = 1 at
    exactly one lead_day, so each sample contributes at most n_observed_lakes rows
    (all at the same lead_day).  init_date is recovered from ds.valid_samples.
    """
    records = []
    model.eval()
    with torch.no_grad():
        for idx in range(len(ds)):
            ...
            # SingleLeadDatasetLake indexes by valid_samples (N, 2);
            # TemporalGraphDatasetLake indexes by valid_starts (N,).
            if hasattr(ds, "valid_samples"):
                ecmwf_j = int(ds.valid_samples[idx, 0])
            else:
                ecmwf_j = int(ds.valid_starts[idx])
            init_date = pd.Timestamp(ds.ecmwf_init_dates[ecmwf_j])
...
def compute_lake_metrics(df: pd.DataFrame, lead_day: int | None = None) -> pd.DataFrame:
    """Compute per-lake metrics (physical units) for one lead day or all lead days."""
...
def main():
    parser = argparse.ArgumentParser(
        description="Single-lead-day temporal-CV LSTM inference — load checkpoint, run predictions, save CSVs"
    )
    ...
    parser.add_argument("--fold-idx",  type=int, required=True,
                        help="Temporal fold to run inference for (0, 1, or 2)")
    parser.add_argument("--lead-assign", default="round_robin",
    ...
    assert forecast_horizon > 1, (
        f"This script is designed for forecast_horizon > 1, got {forecast_horizon}."
    )
    ...
    la_slug    = "rr" if args.lead_assign == "round_robin" else "rand"

    # Mirror run-name construction from training script
    ...
    print(f"Lead assign            : {args.lead_assign}")

    if not run_dir.exists():
        raise FileNotFoundError(
            f"Run directory not found: {run_dir}\n"
            f"Check --run-name, --fold-idx, --lead-assign, and --seed match training.\n"
    ...
    # ── Rebuild the same single-lead split ────────────────────────────────────
    train_ds, val_ds, test_ds, norm_stats = build_temporal_cv_fold_single_lead(
    ...
    print(f"Splits : {len(train_ds)} train (single-lead) | {len(val_ds)} val | {len(test_ds)} test (multi-lead)")
    ...
    print(f"  (test_result_df.csv carries fold_idx={args.fold_idx} on every row)\n")
    ...
    inference_metrics = {
        ...
        "n_train_samples":  int(norm_stats["n_train_samples"]),
    ...
    print(
        "\nTo compute the pooled metric across all three folds, concatenate the "
        "three test_result_df.csv files and compute MSE/RMSE/NSE globally.\n"
        "Each row carries a fold_idx column for groupby diagnostics."
    )

"""
Temporal expanding-window cross-validation for multi-day lake WSE forecasting
using an LSTM-only baseline — single-lead-day training variant.

Identical to run_training_lake_lstm_wsend_temporal_cv.py except the dataset is
built with build_temporal_cv_fold_single_lead, so each SWOT observation date
appears exactly once as a training sample with one assigned lead day.

Lead assignment:
    "round_robin"  d = chronological_position % forecast_horizon  (default)
    "random"       d drawn uniformly from {0..forecast_horizon-1} with --seed

Architecture: InputEncoder → LSTMBlock × N → ForecastHead.
Registry CSV: registry_lstm_wsend_single_lead_cv.csv

Usage:
  python single_lead/run_training_lake_lstm_wsend_single_lead_cv.py \\
...
def train(cfg, args):
    ...
    registry_path = Path(args.save_dir) / "registry_lstm_wsend_single_lead_cv.csv"
    exp_id        = args.run_name.split("_")[0]
    reg_header = [
        "run_name", "exp_id", "seed", "fold_idx", "lead_assign",
        "train_start", "train_end", "test_start", "test_end",
        "forecast_horizon", "n_lakes", "n_train_samples", "n_test_dates",
    ...
    ax.set_xlabel("Epoch")
    ax.set_ylabel(f"Loss — {forecast_horizon}-day forecast (single lead)")
    ax.set_title(
        f"{args.run_name}  —  fold {args.fold_idx} "
        f"[test {fold_def['test_start']} → {fold_def['test_end']}]  "
        f"lead_assign={args.lead_assign}"
    )
    ax.legend()
    plt.tight_layout()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Single-lead-day temporal CV — multi-day lake WSE forecasting (LSTM baseline)"
    )
    parser.add_argument(
        "--config",
    ...
    parser.add_argument("--run-name",  required=True,
                        help="Base run name; fold/horizon/seed are appended automatically")
    parser.add_argument("--fold-idx",  type=int, required=True,
                        help="Which temporal fold to run (0, 1, or 2)")
    parser.add_argument("--lead-assign", default="random",
                        choices=["round_robin", "random"],
                        help="Lead day assignment strategy per obs_date")
    parser.add_argument(
        "--device",

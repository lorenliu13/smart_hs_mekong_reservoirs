#!/bin/bash
#SBATCH --job-name=ablation_noswot_lstm_temporalcv
#SBATCH --output=ablation_noswot_lstm_temporalcv.out
#SBATCH --error=ablation_noswot_lstm_temporalcv.err
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
RUN_NAME="ablation_noswot_$(basename "$CONFIG" .yaml)"
SEED=42
...
for FOLD_IDX in $(seq 0 $((N_FOLDS - 1))); do
    echo "============================================================"
    echo "  Ablation (no SWOT input) — Fold ${FOLD_IDX} / $((N_FOLDS - 1))"
    echo "============================================================"

    # ── Training ───────────────────────────────────────────────────────────────
    python wsend_lstm_training/ablation_noswot/run_training_lake_lstm_wsend_temporal_cv_noswot.py \
        --config          "$CONFIG" \
    ...
    python wsend_lstm_training/ablation_noswot/run_inference_lstm_wsend_temporal_cv_noswot.py \
        --config          "$CONFIG" \
        --wse-datacube    "$WSE_DATACUBE" \
        --era5-datacube   "$ERA5_DATACUBE" \
        --ecmwf-datacube  "$ECMWF_DATACUBE" \
...
echo "All ${N_FOLDS} folds complete (ablation: no SWOT input)."

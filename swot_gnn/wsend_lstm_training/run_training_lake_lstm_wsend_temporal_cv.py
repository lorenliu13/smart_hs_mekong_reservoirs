...
init_dates are split by calendar date into train and test windows; all lakes
are active in every fold (no spatial split).  Run one fold at a time.

Fold definitions (see data/temporal_cv.py — TEMPORAL_FOLD_DATES):
...
Final evaluation metric:
    Pool test predictions from all three folds and compute one global MSE over
    all (sample, lake, lead_day) tuples where obs_mask=1.  Each fold also
    reports its own test_loss for per-period diagnostics.

Key differences from wsend_training/run_training_lake_wsend_temporal_cv.py:
  - Imports _run_epoch_lstm_nd from training.train_lstm_nd (no edge_index / batch).
  - Model call omits edge_index and batch vector.
  - Registry CSV is registry_lstm_wsend_temporal_cv.csv.

Usage:
  python run_training_lake_lstm_wsend_temporal_cv.py \\
...
Launch all folds (bash example):
  for i in 0 1 2; do
    python run_training_lake_lstm_wsend_temporal_cv.py ... --fold-idx $i &
  done
"""
import argparse
import csv
...
def train(cfg, args):
    ...
    # All lakes active — spatial_mask is all-ones (set in build_temporal_cv_fold)
    ...
    registry_path = Path(args.save_dir) / "registry_lstm_wsend_temporal_cv.csv"
    ...
    ax.set_xlabel("Epoch")
    ax.set_ylabel(f"Loss — {forecast_horizon}-day forecast")
    ax.set_title(
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Temporal expanding-window CV — multi-day lake WSE forecasting (LSTM baseline)"
    )
    parser.add_argument(
        "--config",
    ...
    parser.add_argument("--run-name",  required=True,
                        help="Base run name; fold and horizon are appended automatically")
    ...
    assert forecast_horizon > 1, (
        f"run_training_lake_lstm_wsend_temporal_cv.py is designed for forecast_horizon > 1, "
    ...
    fold_def = TEMPORAL_FOLD_DATES[args.fold_idx]
    # Example: exp02_lstm_wsend_temporalcv_fold0of3_h10_lstm_nd_s42

"""
Inference script for temporal expanding-window CV multi-day lake WSE forecasting
using the LSTM-only baseline (no graph).

Mirrors run_inference_wsend_temporal_cv.py but for checkpoints produced by
run_training_lake_lstm_wsend_temporal_cv.py.  Key difference from the GNN
temporal inference script:
  - The model forward call omits edge_index and batch:
      model(x, static_features=static_t)   # LSTM — no graph
    vs.
      model(x, edge_index, static_features=static_t, batch=batch_vec)  # GNN

Everything else (metrics, fold_idx column, denormalization, CSV / JSON output,
arg parser) is identical to the GNN temporal CV inference script.

Dataset reconstruction mirrors training exactly:
  - build_temporal_cv_fold with the same fold_idx / val_frac arguments.
  - require_obs_on_any_forecast_day=True (same default as training).

Pooled evaluation:
  Load test_result_df.csv from all three folds, concatenate, and compute one
  global metric over all (fold, sample, lake, lead_day) tuples.  Each row
  carries a fold_idx column for groupby diagnostics.

Usage:
  python run_inference_lstm_wsend_temporal_cv.py \\
    ...
    --run-name        exp02_lstm_wsend_temporalcv \\
    --fold-idx        0 \\
    --seed            42

Note: --config, --fold-idx, and --seed must match the values used during training
so that the run directory name is resolved correctly.
"""
import argparse
import json
import sys
...
def run_inference(ds, model, device, fold_idx: int, split_name: str = "") -> pd.DataFrame:
    ...
    Each row records:
        ...
        pred_norm, target_norm, pred_std_norm, days_since_last_swot

    fold_idx marks which temporal CV fold this dataset belongs to, enabling
    pooled evaluation by concatenating test_result_df.csv from all three folds.
    ...
    Key difference from GNN temporal CV inference: model is called without
    edge_index — model(x, static_features=static_t).
    """
    ...
    with torch.no_grad():
        for idx in range(len(ds)):
            ...
            # LSTM model: no edge_index or batch argument
...
def compute_lake_metrics(df: pd.DataFrame, lead_day: int | None = None) -> pd.DataFrame:
    ...
    For deterministic models: NSE, KGE, RMSE, MAE.
    For probabilistic models (pred_std_m present): additionally CRPS, cov90, piw90_m.
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="Temporal-CV multi-step LSTM inference — load checkpoint, run predictions, save CSVs"
    )
    ...
    assert forecast_horizon > 1, (
        f"run_inference_lstm_wsend_temporal_cv.py is designed for forecast_horizon > 1, "
        f"got {forecast_horizon}."
    )
    ...
    # Mirror run-name construction from run_training_lake_lstm_wsend_temporal_cv.py
    ...
    if not run_dir.exists():
        raise FileNotFoundError(
            ...
            f"Check that --run-name, --fold-idx, and --seed match the training run.\n"
    ...
    # ── Rebuild the same temporal CV split ────────────────────────────────────

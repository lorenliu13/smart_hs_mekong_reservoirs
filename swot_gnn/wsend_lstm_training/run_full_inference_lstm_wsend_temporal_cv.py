"""
Full inference script for the LSTM-only temporal CV model.

Unlike run_inference_lstm_wsend_temporal_cv.py (which only runs on init dates
...
Usage:
  python run_full_inference_lstm_wsend_temporal_cv.py \\
...
def run_full_inference(ds, model, device, fold_used: int) -> pd.DataFrame:
    """
    Run LSTM over every sample in ds.  Outputs a row for EVERY lake × lead_day,
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Full inference: run the best LSTM checkpoint on ALL init dates, "

"""
Inference script for regional CV multi-day lake WSE forecasting
using the LSTM-only baseline (no graph).

Mirrors wsend_training/run_inference_wsend_regional_cv.py but for checkpoints
produced by run_training_lake_lstm_wsend_regional_cv.py.  Key difference from
the GNN inference script:
...
Everything else (metrics, denormalization, CSV / JSON output, arg parser) is
identical to the GNN inference script.
...
Usage:
  python run_inference_lstm_wsend_regional_cv.py \\
...
name is resolved correctly (identical slug logic).
"""
import argparse
import json
import sys
...
def run_inference(ds, model, device, test_lake_ids: set, split_name=''):
    """Run LSTM model over all samples; return DataFrame of observed-lake predictions.

    Identical to run_inference in run_inference_wsend_regional_cv.py except the
    model is called without edge_index:
        model(x, static_features=static_t)   # LSTM — no graph

    Each row records:
        lake_id, init_date, forecast_date, lead_day, pred_norm, target_norm,
        pred_std_norm (NaN for deterministic models), is_test_lake
    """
    records = []
    model.eval()
    with torch.no_grad():
...
def main():
    parser = argparse.ArgumentParser(
        description="Regional-CV multi-step LSTM inference — load checkpoint, run predictions, save CSVs"
    )
    ...
    assert forecast_horizon > 1, (
        f"run_inference_lstm_wsend_regional_cv.py is designed for forecast_horizon > 1, "
    ...
    test_mask_np  = test_ds.spatial_mask.numpy()
    ...
    model_type     = cfg_model_load.pop("model_type", "LSTMBaselineMultiStep")
    ...
    # ── Run inference on full test_ds (covers all lakes and all dates) ─────────
    print("\nRunning inference ...")
